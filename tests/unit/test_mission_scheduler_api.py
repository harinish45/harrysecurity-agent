...
from web.mission_scheduler_api import _scheduler
...
def test_mission_job_submission_is_scope_checked_and_listed(monkeypatch, tmp_path):
    ...
    # Move the mission into a state that accepts jobs.
    assert client.post(f"/api/missions/{mission_id}/transition", json={"status": "authorized"}).status_code == 200

    queued = client.post(
        f"/api/missions/{mission_id}/jobs",
        json={"capability": "network.port_discovery", "target_scope": ["127.0.0.1"]},
    )
...
def test_mission_job_rejects_scope_outside_mission(monkeypatch, tmp_path):
    ...
    rejected = client.post(
        f"/api/missions/{mission_id}/jobs",
        json={"capability": "network.port_discovery", "target_scope": ["127.0.0.2"]},
    )
