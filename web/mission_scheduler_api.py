...
async def submit_job(mission_id: str, payload: dict[str, Any], request: Request) -> dict[str, Any]:
    ...
    try:
        ...
        if mission.target not in target_scope:
    ...
    try:
        ...
        _service.record(mission_id, "job.queued", actor="mission-api", payload={"job_id": job.job_id, "capability": job.capability, "risk_level": job.risk_level.value})
    except (ValueError, RuntimeError) as exc:
        raise HTTPException(status_code=409, detail=str(exc)) from exc

    return {"job_id": scheduled.job.job_id, "mission_id": scheduled.job.mission_id, "task_id": scheduled.job.task_id, "capability": scheduled.job.capability, "state": scheduled.state.value, "priority": scheduled.priority, "target_scope": list(scheduled.job.target_scope)}
...
async def list_jobs(mission_id: str, request: Request) -> dict[str, Any]:
    ...
    return {"mission_id": mission_id, "jobs": [{"job_id": item.job.job_id, "task_id": item.job.task_id, "capability": item.job.capability, "state": item.state.value, "priority": item.priority, "attempt": item.job.attempt, "retries": item.retries, "error": item.error, "target_scope": list(item.job.target_scope)} for item in items]}
