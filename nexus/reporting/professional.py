...
def _risk_table(counts: dict[str, int]) -> str:
    ...
    for severity in SEVERITIES:
        rows.append(
            f"<tr><td class='sev sev-{severity}'>{severity.title()}</td>"
            f"<td>{counts.get(severity, 0)}</td></tr>"
        )
    return "".join(rows)
...
def render_html(data: dict[str, Any], branding: ReportBranding | None = None) -> str:
    ...
    total = len(findings)
    target = _text(data.get("_meta", {}).get("target") or data.get("target"))
    generated = _text(data.get("_meta", {}).get("generated_at") or data.get("timestamp"))
    authorization = _text(data.get("_meta", {}).get("authorization_reference"), "Recorded in engagement controls")
...
<div class='summary-grid'><div class='kpi'><span class='small'>Total findings</span><b>{total}</b></div><div class='kpi'><span class='small'>Risk band</span><b>{risk_band}</b></div><div class='kpi'><span class='small'>Weighted exposure</span><b>{weighted}</b></div><div class='kpi'><span class='small'>Open ports</span><b>{len(open_ports) if isinstance(open_ports,list) else '—'}</b></div></div>
<table><thead><tr><th>Severity</th><th>Count</th></tr></thead><tbody>{_risk_table(counts)}</tbody></table>
<div class='section'><h3>Assessment posture</h3><p>Prioritize critical and high-severity findings first, then address recurring medium-risk weaknesses and hardening opportunities. Retest resolved findings and retain the associated evidence for closure validation.</p></div>
</section>
<section class='pagebreak'><h2>Engagement scope & methodology</h2><table><tbody><tr><th>Target</th><td>{target}</td></tr><tr><th>Authorization</th><td>{authorization}</td></tr><tr><th>Open services</th><td>{_text(ports,'None recorded')}</td></tr><tr><th>Evidence model</th><td>Normalized observations with preserved provenance</td></tr><tr><th>Finding model</th><td>Severity, confidence, asset, evidence and remediation</td></tr></tbody></table><div class='section'><h3>Professional handling notes</h3><p>Raw tool output is treated as untrusted data. Report content is generated deterministically from the assessment snapshot. AI-generated reasoning must not be represented as confirmed exploitation unless the underlying evidence supports that conclusion.</p></div></section>
<section class='pagebreak'><h2>Findings</h2><p class='small'>{total} finding(s) included. Severity and confidence should be interpreted together with business criticality and validated attack paths.</p>{cards}</section>
