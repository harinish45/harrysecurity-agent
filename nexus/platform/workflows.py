    ...
    def critical_path(self) -> list[str]:
        ...
        while remaining:
            candidates = [
                ...
                if task_id in remaining and all(item not in remaining for item in task.depends_on)
            ]
            if not candidates:
...
workflows.register(WorkflowSpec(
    ...
    (
        ...
        WorkflowTask("observe", "Collect defensive observations", "agents.detection_engineering", depends_on=("attack_sim",), priority=90),
        WorkflowTask("gap", "Analyze detection gap", "agents.purple_team", depends_on=("observe",), priority=80),
        WorkflowTask("improve", "Generate detection improvement", "defensive.detection_bridge", depends_on=("gap",), priority=70),
...
workflows.register(WorkflowSpec(
    ...
    (
        ...
        WorkflowTask("patch", "Remediation guidance", "reporting.developer_report", depends_on=("evidence",), priority=50),
    ),
))
...
workflows.register(WorkflowSpec(
    ...
    (
        ...
        WorkflowTask("walkthrough", "Reproducible walkthrough", "reporting.technical_report", depends_on=("validate",), priority=40),
    ),
))
