from nexus.agents.base_agent import BaseAgent
from nexus.tools.registry import tool_registry
from nexus.foundation.schema import STATUS_COMPLETED, STATUS_NO_FINDINGS, STATUS_FAILED, tool_result


class IotAgent(BaseAgent):
    name = "iot_agent"
    description = "specialized agent for IoT — smart device assessment, CAN bus, firmware extraction, and embedded Linux"

    async def run(self, task: str, target: str = "", **kwargs) -> dict:
        if not target:
            return tool_result(self.name, target or "unknown", status=STATUS_FAILED, error="No target specified")

        findings = []
        tools_used = []

        try:
            result = tool_registry.run("iot.smart_device_assessment", target=target)
            tools_used.append("iot.smart_device_assessment")
            if result.get("findings"):
                findings.extend(result["findings"])
        except Exception as e:
            findings.append({"title": f"Smart device assessment error: {e}", "severity": "low", "confidence": "medium"})

        try:
            result = tool_registry.run("iot.can_bus_testing", target=target)
            tools_used.append("iot.can_bus_testing")
            if result.get("findings"):
                findings.extend(result["findings"])
        except Exception as e:
            findings.append({"title": f"CAN bus testing error: {e}", "severity": "low", "confidence": "medium"})

        try:
            result = tool_registry.run("iot.firmware_extraction", target=target)
            tools_used.append("iot.firmware_extraction")
            if result.get("findings"):
                findings.extend(result["findings"])
        except Exception as e:
            findings.append({"title": f"Firmware extraction error: {e}", "severity": "low", "confidence": "medium"})

        try:
            result = tool_registry.run("iot.embedded_linux", target=target)
            tools_used.append("iot.embedded_linux")
            if result.get("findings"):
                findings.extend(result["findings"])
        except Exception as e:
            findings.append({"title": f"Embedded Linux error: {e}", "severity": "low", "confidence": "medium"})

        try:
            result = tool_registry.run("iot.jtag_analysis", target=target)
            tools_used.append("iot.jtag_analysis")
            if result.get("findings"):
                findings.extend(result["findings"])
        except Exception as e:
            findings.append({"title": f"JTAG analysis error: {e}", "severity": "low", "confidence": "medium"})

        try:
            result = tool_registry.run("iot.uart_analysis", target=target)
            tools_used.append("iot.uart_analysis")
            if result.get("findings"):
                findings.extend(result["findings"])
        except Exception as e:
            findings.append({"title": f"UART analysis error: {e}", "severity": "low", "confidence": "medium"})

        status = STATUS_COMPLETED if findings else STATUS_NO_FINDINGS
        return tool_result(
            self.name, target,
            status=status,
            findings=findings,
            summary=f"IoT security testing completed for {target} using {len(tools_used)} tools, {len(findings)} findings",
            metadata={"tools_used": tools_used},
        )