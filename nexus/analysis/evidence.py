...
def correlate(evidence: Iterable[Evidence]) -> tuple[CorrelatedFinding, ...]:
    ...
    for key, items in groups.items():
        ...
        severity = max((item.severity.lower() for item in ordered), key=lambda value: severity_rank.get(value, -1))
    ...
    return tuple(sorted(findings, key=lambda item: (-severity_rank.get(item.severity, -1), item.finding_id)))
