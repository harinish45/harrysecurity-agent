#!/usr/bin/env python3
"""
NEXUS-STRIKE — forensics tool: Browser Forensics
Domain: forensics
"""
...
def run(target: str, **kwargs: Any) -> dict[str, Any]:
    """forensics tool: Browser Forensics"""
    findings: list[Finding] = []

    if not target or not target.strip():
        return tool_result("forensics.browser_forensics", target, status=STATUS_FAILED, error="Empty target")
    ...
    try:
        ...
        return tool_result("forensics.browser_forensics", target, status=status, findings=findings)
...
tool_registry.register("forensics.browser_forensics", run, metadata={
    ...
    "domain": "forensics",
    "status": "completed",
    "description": "forensics tool: Browser Forensics",
    "parameters": {
