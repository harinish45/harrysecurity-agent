...
def run(target: str, **kwargs: Any) -> dict[str, Any]:
    ...
    if not target or not target.strip():
        return tool_result("forensics.email_forensics", target, status=STATUS_FAILED, error="Empty target")
    ...
    try:
        ...
        return tool_result("forensics.email_forensics", target, status=status, findings=findings)
