...
def run(target: str, **kwargs: Any) -> dict[str, Any]:
    ...
    if not target or not target.strip():
        return tool_result("hardware.rubber_ducky_testing", target, status=STATUS_FAILED, error="Empty target")
    ...
    try:
        try:
            ...
            findings.append(Finding(
                ...
                tool="hardware.rubber_ducky_testing",
            ))
        except Exception:
            pass
        ...
        return tool_result("hardware.rubber_ducky_testing", target, status=status, findings=findings)

    except Exception as e:
        return tool_result("hardware.rubber_ducky_testing", target, status=STATUS_FAILED, error=str(e))
