...
def run(target: str, **kwargs: Any) -> dict[str, Any]:
    ...
    if not target or not target.strip():
        return tool_result("wireless.wifi_audit", target, status=STATUS_FAILED, error="Empty target")
    ...
    try:
        ...
        return tool_result("wireless.wifi_audit", target, status=status, findings=findings)

    except Exception as e:
        return tool_result("wireless.wifi_audit", target, status=STATUS_FAILED, error=str(e))
...
tool_registry.register("wireless.wifi_audit", run, metadata={
    ...
    "domain": "wireless",
    "status": "completed",
    "description": "wireless tool: Wifi Audit",
    "parameters": {
