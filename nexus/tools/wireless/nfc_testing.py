...
def run(target: str, **kwargs: Any) -> dict[str, Any]:
    ...
    if not target or not target.strip():
        return tool_result("wireless.nfc_testing", target, status=STATUS_FAILED, error="Empty target")
    ...
    try:
        ...
        return tool_result("wireless.nfc_testing", target, status=status, findings=findings)

    except Exception as e:
        return tool_result("wireless.nfc_testing", target, status=STATUS_FAILED, error=str(e))
...
tool_registry.register("wireless.nfc_testing", run, metadata={
    ...
    "domain": "wireless",
    "status": "completed",
    "description": "wireless tool: Nfc Testing",
    "parameters": {
