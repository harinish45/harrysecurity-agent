...
def run(target: str, **kwargs: Any) -> dict[str, Any]:
    ...
    if not target or not target.strip():
        return tool_result("wireless.rogue_ap", target, status=STATUS_FAILED, error="Empty target")
    ...
    try:
        ...
        return tool_result("wireless.rogue_ap", target, status=status, findings=findings)

    except Exception as e:
        return tool_result("wireless.rogue_ap", target, status=STATUS_FAILED, error=str(e))
...
tool_registry.register("wireless.rogue_ap", run, metadata={
    ...
    "domain": "wireless",
    "status": "completed",
    "description": "wireless tool: Rogue Ap",
    "parameters": {
