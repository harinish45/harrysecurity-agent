#!/usr/bin/env python3
"""
NEXUS-STRIKE — network tool: Network Map
Domain: network
"""
...
def run(target: str, **kwargs: Any) -> dict[str, Any]:
    ...
    try:
        ...
        return tool_result("network.network_map", target, status=status, findings=findings)

    except Exception as e:
        return tool_result("network.network_map", target, status=STATUS_FAILED, error=str(e))
...
tool_registry.register("network.network_map", run, metadata={
    ...
    "domain": "network",
    "status": "completed",
    "description": "network tool: Network Map",
    "parameters": {
