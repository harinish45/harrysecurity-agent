#!/usr/bin/env python3
"""
NEXUS-STRIKE — reverse_engineering tool: Function Analysis
...
def run(target: str, **kwargs: Any) -> dict[str, Any]:
    """reverse_engineering tool: Function Analysis"""
    findings: list[Finding] = []

    if not target or not target.strip():
        return tool_result("reverse_engineering.function_analysis", target, status=STATUS_FAILED, error="Empty target")
    ...
    try:
        ...
        return tool_result("reverse_engineering.function_analysis", target, status=status, findings=findings)
...
tool_registry.register("reverse_engineering.function_analysis", run, metadata={
    ...
    "domain": "reverse_engineering",
    "status": "completed",
    "description": "reverse_engineering tool: Function Analysis",
    "parameters": {
