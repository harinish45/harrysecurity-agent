#!/usr/bin/env python3
"""
NEXUS-STRIKE — iam tool: Sso Testing
Domain: iam
"""
...
def run(target: str, **kwargs: Any) -> dict[str, Any]:
    """iam tool: Sso Testing"""
    findings: list[Finding] = []

    if not target or not target.strip():
        return tool_result("iam.sso_testing", target, status=STATUS_FAILED, error="Empty target")
    ...
    try:
        ...
        return tool_result("iam.sso_testing", target, status=status, findings=findings)

    except Exception as e:
        return tool_result("iam.sso_testing", target, status=STATUS_FAILED, error=str(e))
...
tool_registry.register("iam.sso_testing", run, metadata={
    ...
    "domain": "iam",
    "status": "completed",
    "description": "iam tool: Sso Testing",
    "parameters": {
