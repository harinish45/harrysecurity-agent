...
def run(target: str, **kwargs: Any) -> dict[str, Any]:
    ...
    if not target or not target.strip():
        return tool_result("iam.oauth_testing", target, status=STATUS_FAILED, error="Empty target")
    ...
    try:
        ...
        return tool_result("iam.oauth_testing", target, status=status, findings=findings)

    except Exception as e:
        return tool_result("iam.oauth_testing", target, status=STATUS_FAILED, error=str(e))
