...
def run(target: str, **kwargs: Any) -> dict[str, Any]:
    ...
    if not target or not target.strip():
        return tool_result("threat_intel.threat_actor_profiling", target, status=STATUS_FAILED, error="Empty target")
    ...
    try:
        try:
            ...
            findings.append(Finding(
                ...
                tool="threat_intel.threat_actor_profiling",
            ))
        except Exception:
            pass
        ...
        return tool_result("threat_intel.threat_actor_profiling", target, status=status, findings=findings)

    except Exception as e:
        return tool_result("threat_intel.threat_actor_profiling", target, status=STATUS_FAILED, error=str(e))
