...
def run(target: str, **kwargs: Any) -> dict[str, Any]:
    ...
    if not target or not target.strip():
        return tool_result("blue_team.log_review", target, status=STATUS_FAILED, error="Empty target")
    ...
    try:
        ...
        return tool_result("blue_team.log_review", target, status=status, findings=findings)

    except Exception as e:
        return tool_result("blue_team.log_review", target, status=STATUS_FAILED, error=str(e))
...
tool_registry.register("blue_team.log_review", run, metadata={
    ...
    "domain": "blue_team",
    "status": "completed",
    "description": "blue_team tool: Log Review",
    "parameters": {
