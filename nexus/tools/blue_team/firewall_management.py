...
def run(target: str, **kwargs: Any) -> dict[str, Any]:
    ...
    if not target or not target.strip():
        return tool_result("blue_team.firewall_management", target, status=STATUS_FAILED, error="Empty target")
    ...
    try:
        try:
            ...
            findings.append(Finding(
                ...
                tool="blue_team.firewall_management",
            ))
        except Exception:
            pass
        ...
        return tool_result("blue_team.firewall_management", target, status=status, findings=findings)

    except Exception as e:
        return tool_result("blue_team.firewall_management", target, status=STATUS_FAILED, error=str(e))
