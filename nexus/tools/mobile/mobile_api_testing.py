...
def run(target: str, **kwargs: Any) -> dict[str, Any]:
    """mobile tool: Mobile Api Testing"""
    findings: list[Finding] = []

    if not target or not target.strip():
        return tool_result("mobile.mobile_api_testing", target, status=STATUS_FAILED, error="Empty target")
    ...
    try:
        ...
        return tool_result("mobile.mobile_api_testing", target, status=status, findings=findings)

    except Exception as e:
        return tool_result("mobile.mobile_api_testing", target, status=STATUS_FAILED, error=str(e))
...
tool_registry.register("mobile.mobile_api_testing", run, metadata={
    ...
    "domain": "mobile",
    "status": "completed",
    "description": "mobile tool: Mobile Api Testing",
    "parameters": {
