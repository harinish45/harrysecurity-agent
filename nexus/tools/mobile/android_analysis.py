...
def run(target: str, **kwargs: Any) -> dict[str, Any]:
    """mobile tool: Android Analysis"""
    findings: list[Finding] = []

    if not target or not target.strip():
        return tool_result("mobile.android_analysis", target, status=STATUS_FAILED, error="Empty target")
    ...
    try:
        ...
        return tool_result("mobile.android_analysis", target, status=status, findings=findings)

    except Exception as e:
        return tool_result("mobile.android_analysis", target, status=STATUS_FAILED, error=str(e))
...
tool_registry.register("mobile.android_analysis", run, metadata={
    ...
    "domain": "mobile",
    "status": "completed",
    "description": "mobile tool: Android Analysis",
    "parameters": {
