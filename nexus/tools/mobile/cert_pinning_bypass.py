...
def run(target: str, **kwargs: Any) -> dict[str, Any]:
    """mobile tool: Cert Pinning Bypass"""
    findings: list[Finding] = []

    if not target or not target.strip():
        return tool_result("mobile.cert_pinning_bypass", target, status=STATUS_FAILED, error="Empty target")
    ...
    try:
        ...
        return tool_result("mobile.cert_pinning_bypass", target, status=status, findings=findings)

    except Exception as e:
        return tool_result("mobile.cert_pinning_bypass", target, status=STATUS_FAILED, error=str(e))
...
tool_registry.register("mobile.cert_pinning_bypass", run, metadata={
    ...
    "domain": "mobile",
    "status": "completed",
    "description": "mobile tool: Cert Pinning Bypass",
    "parameters": {
