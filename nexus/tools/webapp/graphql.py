#!/usr/bin/env python3
"""
NEXUS-STRIKE — webapp tool: Graphql
Domain: webapp
"""
...
def run(target: str, **kwargs: Any) -> dict[str, Any]:
    ...
    try:
        ...
        return tool_result("webapp.graphql", target, status=status, findings=findings)

    except Exception as e:
        return tool_result("webapp.graphql", target, status=STATUS_FAILED, error=str(e))
...
tool_registry.register("webapp.graphql", run, metadata={
    ...
    "domain": "webapp",
    "status": "completed",
    "description": "webapp tool: Graphql",
    "parameters": {
