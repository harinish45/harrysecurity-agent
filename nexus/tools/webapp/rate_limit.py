#!/usr/bin/env python3
"""
NEXUS-STRIKE — webapp tool: Rate Limit
Domain: webapp
"""
...
def run(target: str, **kwargs: Any) -> dict[str, Any]:
    ...
    try:
        ...
        return tool_result("webapp.rate_limit", target, status=status, findings=findings)

    except Exception as e:
        return tool_result("webapp.rate_limit", target, status=STATUS_FAILED, error=str(e))
...
tool_registry.register("webapp.rate_limit", run, metadata={
    ...
    "domain": "webapp",
    "status": "completed",
    "description": "webapp tool: Rate Limit",
    "parameters": {
