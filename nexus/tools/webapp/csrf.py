...
def run(target: str, **kwargs: Any) -> dict[str, Any]:
    ...
    try:
        ...
        return tool_result("webapp.csrf", target, status=status, findings=findings)

    except Exception as e:
        return tool_result("webapp.csrf", target, status=STATUS_FAILED, error=str(e))
