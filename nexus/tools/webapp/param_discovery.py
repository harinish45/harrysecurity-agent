#!/usr/bin/env python3
"""
NEXUS-STRIKE — webapp tool: Param Discovery
Domain: webapp
"""
...
def run(target: str, **kwargs: Any) -> dict[str, Any]:
    ...
    try:
        ...
        return tool_result("webapp.param_discovery", target, status=status, findings=findings)

    except Exception as e:
        return tool_result("webapp.param_discovery", target, status=STATUS_FAILED, error=str(e))
...
tool_registry.register("webapp.param_discovery", run, metadata={
    ...
    "domain": "webapp",
    "status": "completed",
    "description": "webapp tool: Param Discovery",
    "parameters": {
