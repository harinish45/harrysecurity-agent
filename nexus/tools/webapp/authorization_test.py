#!/usr/bin/env python3
"""
NEXUS-STRIKE — webapp tool: Authorization Test
Domain: webapp
"""
...
def run(target: str, **kwargs: Any) -> dict[str, Any]:
    """webapp tool: Authorization Test"""
    ...
    try:
        ...
        return tool_result("webapp.authorization_test", target, status=status, findings=findings)

    except Exception as e:
        return tool_result("webapp.authorization_test", target, status=STATUS_FAILED, error=str(e))
...
tool_registry.register("webapp.authorization_test", run, metadata={
    ...
    "domain": "webapp",
    "status": "completed",
    "description": "webapp tool: Authorization Test",
    "parameters": {
