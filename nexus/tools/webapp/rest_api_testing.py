#!/usr/bin/env python3
"""
NEXUS-STRIKE — webapp tool: Rest Api Testing
Domain: webapp
"""
...
def run(target: str, **kwargs: Any) -> dict[str, Any]:
    ...
    try:
        ...
        return tool_result("webapp.rest_api_testing", target, status=status, findings=findings)

    except Exception as e:
        return tool_result("webapp.rest_api_testing", target, status=STATUS_FAILED, error=str(e))
...
tool_registry.register("webapp.rest_api_testing", run, metadata={
    ...
    "domain": "webapp",
    "status": "completed",
    "description": "webapp tool: Rest Api Testing",
    "parameters": {
