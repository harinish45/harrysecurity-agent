#!/usr/bin/env python3
"""
NEXUS-STRIKE — webapp tool: Browser Agent
Domain: webapp
"""
...
def run(target: str, **kwargs: Any) -> dict[str, Any]:
    ...
    try:
        ...
        return tool_result("webapp.browser_agent", target, status=status, findings=findings)

    except Exception as e:
        return tool_result("webapp.browser_agent", target, status=STATUS_FAILED, error=str(e))
...
tool_registry.register("webapp.browser_agent", run, metadata={
    ...
    "domain": "webapp",
    "status": "completed",
    "description": "webapp tool: Browser Agent",
    "parameters": {
