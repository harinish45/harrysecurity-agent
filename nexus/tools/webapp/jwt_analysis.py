#!/usr/bin/env python3
"""
NEXUS-STRIKE — webapp tool: Jwt Analysis
Domain: webapp
"""
...
def run(target: str, **kwargs: Any) -> dict[str, Any]:
    ...
    try:
        ...
        return tool_result("webapp.jwt_analysis", target, status=status, findings=findings)

    except Exception as e:
        return tool_result("webapp.jwt_analysis", target, status=STATUS_FAILED, error=str(e))
...
tool_registry.register("webapp.jwt_analysis", run, metadata={
    ...
    "domain": "webapp",
    "status": "completed",
    "description": "webapp tool: Jwt Analysis",
    "parameters": {
