#!/usr/bin/env python3
"""
NEXUS-STRIKE — webapp tool: File Upload
Domain: webapp
"""
...
def run(target: str, **kwargs: Any) -> dict[str, Any]:
    ...
    try:
        ...
        return tool_result("webapp.file_upload", target, status=status, findings=findings)

    except Exception as e:
        return tool_result("webapp.file_upload", target, status=STATUS_FAILED, error=str(e))
...
tool_registry.register("webapp.file_upload", run, metadata={
    ...
    "domain": "webapp",
    "status": "completed",
    "description": "webapp tool: File Upload",
    "parameters": {
