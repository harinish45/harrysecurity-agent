...
def run(target: str, **kwargs: Any) -> dict[str, Any]:
    ...
    if not target or not target.strip():
        return tool_result("ai_security.adversarial_ml", target, status=STATUS_FAILED, error="Empty target")
    ...
    try:
        try:
            ...
            findings.append(Finding(
                ...
                tool="ai_security.adversarial_ml",
            ))
        except Exception:
            pass
        ...
        return tool_result("ai_security.adversarial_ml", target, status=status, findings=findings)

    except Exception as e:
        return tool_result("ai_security.adversarial_ml", target, status=STATUS_FAILED, error=str(e))
