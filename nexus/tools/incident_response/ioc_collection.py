...
def run(target: str, **kwargs: Any) -> dict[str, Any]:
    ...
    if not target or not target.strip():
        return tool_result("incident_response.ioc_collection", target, status=STATUS_FAILED, error="Empty target")
    ...
    try:
        try:
            ...
            findings.append(Finding(
                ...
                tool="incident_response.ioc_collection",
            ))
        except Exception:
            pass
        ...
        return tool_result("incident_response.ioc_collection", target, status=status, findings=findings)

    except Exception as e:
        return tool_result("incident_response.ioc_collection", target, status=STATUS_FAILED, error=str(e))
...
tool_registry.register("incident_response.ioc_collection", run, metadata={
    ...
    "domain": "incident_response",
    "status": "completed",
    "description": "incident_response tool: Ioc Collection",
    "parameters": {
