#!/usr/bin/env python3
"""
NEXUS-STRIKE — active_directory tool: Pass The Hash
...
def run(target: str, **kwargs: Any) -> dict[str, Any]:
    """active_directory tool: Pass The Hash"""
    findings: list[Finding] = []

    if not target or not target.strip():
        return tool_result("active_directory.pass_the_hash", target, status=STATUS_FAILED, error="Empty target")
    ...
    try:
        ...
        return tool_result("active_directory.pass_the_hash", target, status=status, findings=findings)
...
tool_registry.register("active_directory.pass_the_hash", run, metadata={
    ...
    "domain": "active_directory",
    "status": "completed",
    "description": "active_directory tool: Pass The Hash",
    "parameters": {
