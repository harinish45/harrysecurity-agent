#!/usr/bin/env python3
"""
NEXUS-STRIKE — active_directory tool: Bloodhound
...
def run(target: str, **kwargs: Any) -> dict[str, Any]:
    """active_directory tool: Bloodhound"""
    findings: list[Finding] = []

    if not target or not target.strip():
        return tool_result("active_directory.bloodhound", target, status=STATUS_FAILED, error="Empty target")
    ...
    try:
        ...
        return tool_result("active_directory.bloodhound", target, status=status, findings=findings)
...
tool_registry.register("active_directory.bloodhound", run, metadata={
    ...
    "domain": "active_directory",
    "status": "completed",
    "description": "active_directory tool: Bloodhound",
    "parameters": {
