...
def run(target: str, **kwargs: Any) -> dict[str, Any]:
    ...
    if not target or not target.strip():
        return tool_result("soc.soar_automation", target, status=STATUS_FAILED, error="Empty target")
    ...
    try:
        ...
        return tool_result("soc.soar_automation", target, status=status, findings=findings)

    except Exception as e:
        return tool_result("soc.soar_automation", target, status=STATUS_FAILED, error=str(e))
...
tool_registry.register("soc.soar_automation", run, metadata={
    ...
    "domain": "soc",
    "status": "completed",
    "description": "soc tool: Soar Automation",
    "parameters": {
