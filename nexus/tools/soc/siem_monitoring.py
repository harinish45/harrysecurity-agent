...
def run(target: str, **kwargs: Any) -> dict[str, Any]:
    ...
    if not target or not target.strip():
        return tool_result("soc.siem_monitoring", target, status=STATUS_FAILED, error="Empty target")
    ...
    try:
        ...
        return tool_result("soc.siem_monitoring", target, status=status, findings=findings)

    except Exception as e:
        return tool_result("soc.siem_monitoring", target, status=STATUS_FAILED, error=str(e))
...
tool_registry.register("soc.siem_monitoring", run, metadata={
    ...
    "domain": "soc",
    "status": "completed",
    "description": "soc tool: Siem Monitoring",
    "parameters": {
