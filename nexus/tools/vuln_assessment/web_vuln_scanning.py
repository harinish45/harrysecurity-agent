...
def run(target: str, **kwargs: Any) -> dict[str, Any]:
    ...
    if not target or not target.strip():
        return tool_result("vuln_assessment.web_vuln_scanning", target, status=STATUS_FAILED, error="Empty target")
    ...
    try:
        try:
            ...
            findings.append(Finding(
                ...
                tool="vuln_assessment.web_vuln_scanning",
            ))
        except Exception:
            pass
        ...
        return tool_result("vuln_assessment.web_vuln_scanning", target, status=status, findings=findings)

    except Exception as e:
        return tool_result("vuln_assessment.web_vuln_scanning", target, status=STATUS_FAILED, error=str(e))
