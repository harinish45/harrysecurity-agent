...
def run(target: str, **kwargs: Any) -> dict[str, Any]:
    ...
    if not target or not target.strip():
        return tool_result("reconnaissance.whois_lookup", target, status=STATUS_FAILED, error="Empty target")
    ...
    try:
        try:
            ...
            findings.append(Finding(
                ...
                tool="reconnaissance.whois_lookup",
            ))
        except Exception:
            pass
        ...
        return tool_result("reconnaissance.whois_lookup", target, status=status, findings=findings)

    except Exception as e:
        return tool_result("reconnaissance.whois_lookup", target, status=STATUS_FAILED, error=str(e))
