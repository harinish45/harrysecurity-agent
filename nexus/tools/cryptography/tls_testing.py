...
def run(target: str, **kwargs: Any) -> dict[str, Any]:
    ...
    if not target or not target.strip():
        return tool_result("cryptography.tls_testing", target, status=STATUS_FAILED, error="Empty target")
    ...
    try:
        ...
        return tool_result("cryptography.tls_testing", target, status=status, findings=findings)

    except Exception as e:
        return tool_result("cryptography.tls_testing", target, status=STATUS_FAILED, error=str(e))
...
tool_registry.register("cryptography.tls_testing", run, metadata={
    ...
    "domain": "cryptography",
    "status": "completed",
    "description": "cryptography tool: Tls Testing",
    "parameters": {
