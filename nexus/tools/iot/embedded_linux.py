#!/usr/bin/env python3
"""
NEXUS-STRIKE — iot tool: Embedded Linux
Domain: iot
"""
...
def run(target: str, **kwargs: Any) -> dict[str, Any]:
    """iot tool: Embedded Linux"""
    findings: list[Finding] = []

    if not target or not target.strip():
        return tool_result("iot.embedded_linux", target, status=STATUS_FAILED, error="Empty target")
    ...
    try:
        ...
        return tool_result("iot.embedded_linux", target, status=status, findings=findings)

    except Exception as e:
        return tool_result("iot.embedded_linux", target, status=STATUS_FAILED, error=str(e))
...
tool_registry.register("iot.embedded_linux", run, metadata={
    ...
    "domain": "iot",
    "status": "completed",
    "description": "iot tool: Embedded Linux",
    "parameters": {
