...
def run(target: str, **kwargs: Any) -> dict[str, Any]:
    ...
    if not target or not target.strip():
        return tool_result("iot.firmware_extraction", target, status=STATUS_FAILED, error="Empty target")
    ...
    try:
        ...
        return tool_result("iot.firmware_extraction", target, status=status, findings=findings)

    except Exception as e:
        return tool_result("iot.firmware_extraction", target, status=STATUS_FAILED, error=str(e))
...
tool_registry.register("iot.firmware_extraction", run, metadata={
    ...
    "domain": "iot",
    "status": "completed",
    "description": "iot tool: Firmware Extraction",
    "parameters": {
