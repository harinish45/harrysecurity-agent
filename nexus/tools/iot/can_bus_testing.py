#!/usr/bin/env python3
"""
NEXUS-STRIKE — iot tool: Can Bus Testing
Domain: iot
"""
...
def run(target: str, **kwargs: Any) -> dict[str, Any]:
    """iot tool: Can Bus Testing"""
    findings: list[Finding] = []

    if not target or not target.strip():
        return tool_result("iot.can_bus_testing", target, status=STATUS_FAILED, error="Empty target")
    ...
    try:
        ...
        return tool_result("iot.can_bus_testing", target, status=status, findings=findings)

    except Exception as e:
        return tool_result("iot.can_bus_testing", target, status=STATUS_FAILED, error=str(e))
...
tool_registry.register("iot.can_bus_testing", run, metadata={
    ...
    "domain": "iot",
    "status": "completed",
    "description": "iot tool: Can Bus Testing",
    "parameters": {
