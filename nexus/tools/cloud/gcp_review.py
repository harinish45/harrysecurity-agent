#!/usr/bin/env python3
"""
NEXUS-STRIKE — cloud tool: Gcp Review
Domain: cloud
"""
...
def run(target: str, **kwargs: Any) -> dict[str, Any]:
    """cloud tool: Gcp Review"""
    findings: list[Finding] = []

    if not target or not target.strip():
        return tool_result("cloud.gcp_review", target, status=STATUS_FAILED, error="Empty target")
    ...
    try:
        ...
        return tool_result("cloud.gcp_review", target, status=status, findings=findings)

    except Exception as e:
        return tool_result("cloud.gcp_review", target, status=STATUS_FAILED, error=str(e))
...
tool_registry.register("cloud.gcp_review", run, metadata={
    ...
    "domain": "cloud",
    "status": "completed",
    "description": "cloud tool: Gcp Review",
    "parameters": {
