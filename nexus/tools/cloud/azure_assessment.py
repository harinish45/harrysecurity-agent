#!/usr/bin/env python3
"""
NEXUS-STRIKE — cloud tool: Azure Assessment
Domain: cloud
"""
...
def run(target: str, **kwargs: Any) -> dict[str, Any]:
    """cloud tool: Azure Assessment"""
    findings: list[Finding] = []

    if not target or not target.strip():
        return tool_result("cloud.azure_assessment", target, status=STATUS_FAILED, error="Empty target")
    ...
    try:
        ...
        return tool_result("cloud.azure_assessment", target, status=status, findings=findings)

    except Exception as e:
        return tool_result("cloud.azure_assessment", target, status=STATUS_FAILED, error=str(e))
...
tool_registry.register("cloud.azure_assessment", run, metadata={
    ...
    "domain": "cloud",
    "status": "completed",
    "description": "cloud tool: Azure Assessment",
    "parameters": {
