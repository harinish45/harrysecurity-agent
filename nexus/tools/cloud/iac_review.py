#!/usr/bin/env python3
"""
NEXUS-STRIKE — cloud tool: Iac Review
Domain: cloud
"""
...
def run(target: str, **kwargs: Any) -> dict[str, Any]:
    ...
    try:
        ...
        return tool_result("cloud.iac_review", target, status=status, findings=findings)

    except Exception as e:
        return tool_result("cloud.iac_review", target, status=STATUS_FAILED, error=str(e))
...
tool_registry.register("cloud.iac_review", run, metadata={
    ...
    "domain": "cloud",
    "status": "completed",
    "description": "cloud tool: Iac Review",
    "parameters": {
