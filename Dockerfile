...
# wheel/msgpack (and pip's own dependency jaraco.context) at whatever
# versions were current when that base image was built -- these stay
# present in the final image (nexus.agents' InstallerAgent shells out to
# `pip install` at runtime to fetch missing packages on demand, so pip
# can't simply be stripped out entirely). Trivy's image scan caught real
# HIGH-severity CVEs here across two iterations as each upgrade uncovered
# the next: wheel 0.45.1 (CVE-2026-24049, code execution via a malicious
# wheel file) and jaraco.context 5.3.0 (CVE-2026-23949, path traversal via
# a malicious tar archive) were fixed by the first `--upgrade pip
# setuptools wheel` pass; that pass itself landed setuptools 70.3.0
# (CVE-2025-47273, path traversal in setuptools' PackageIndex -- not the
# actual latest despite `--upgrade`, since apt/base-image pins can win
# over a bare version-less upgrade) and exposed msgpack 1.1.2
# (GHSA-6v7p-g79w-8964, out-of-bounds read/crash on Unpacker reuse, a pip
# dependency). Pinned floors below rather than a bare `--upgrade` so a
# future base-image bump can't silently reintroduce a "not actually
# latest" version of any of these again. None of these came from
